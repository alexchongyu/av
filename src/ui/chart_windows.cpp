...
// Compute stats for u8 image (one channel: offset 0/1/2 in RGBA8)
static ChannelStats compute_channel_stats_u8(const uint8_t* pixels, int npix, int ch_offset) {
    ...
    // Pass 2: variance, skewness, kurtosis
    double mu = s.mean;
    double sd2 = 0.0, sd3 = 0.0, sd4 = 0.0;
    for (int i = 0; i < npix; ++i) {
...
// Compute stats for f32 image (1024-bin histogram for median/entropy)
...
static ImageStats compute_image_stats(const ImageEntry& img) {
    ImageStats st;
...
static ImageStats compute_diff_stats(const ImageEntry& imgA, const ImageEntry& imgB,
    ...
    if (use_hdr) {
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            extra.psnr[c]      = (extra.mse[c] < 1e-20) ? 999.0 :
                                  10.0 * std::log10(1.0 / extra.mse[c]);
        }
    } else if (use_u8) {
        std::vector<uint8_t> diff_buf(npix * 4, 0);
        for (int i = 0; i < npix; ++i)
            for (int c = 0; c < 3; ++c)
                diff_buf[i*4+c] = (uint8_t)std::clamp(
                    std::abs((int)imgA.pixels[i*4+c] - (int)imgB.pixels[i*4+c]), 0, 255);
        for (int c = 0; c < 3; ++c) {
            ...
            extra.mae[c]       = mae / npix;
            extra.max_error[c] = maxe;
            extra.psnr[c]      = (extra.mse[c] < 1e-10) ? 999.0 :
                                  10.0 * std::log10(255.0 * 255.0 / extra.mse[c]);
        }
    } else return st;
