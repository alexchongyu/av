...
## C++ hex 이스케이프가 뒤따르는 hex-digit 문자를 흡수한다 (Δ/→ UTF-8)

**증상**: `"\xce\x94CCT"`(ΔCCT) 컴파일 시 `error: hex escape sequence out of range`.
`"\xe2\x86\x92B"`(→B), `"\xce\x94E76"`(ΔE76)도 동일.

**원인**: `\x` 이스케이프는 **길이 제한이 없어** 뒤따르는 모든 hex-digit(0-9A-Fa-f)을 계속 삼킨다.
`\x94` 다음이 `C`/`B`/`E`(전부 유효 hex-digit)면 `\x94C` = 0x94C 로 파싱 → char 범위 초과.
`\x94u`(u는 hex 아님)는 정상. 즉 UTF-8 멀티바이트 이스케이프 뒤에 hex-digit 글자가 오면 터진다.

**해결**: 소스가 UTF-8이면 이스케이프 대신 **리터럴 문자 직접 사용**(`"Δu'v'  ΔCCT"`, `"A→B"`).
이 리포는 이미 한글/박스문자 UTF-8이라 안전. (대안: 인접 문자열 리터럴 분리 `"\xce\x94" "CCT"`.)
**적용**: 문자열에 `\x..` UTF-8 이스케이프를 쓸 거면 그냥 리터럴 글자로 쓸 것.

---

## 박스 저역통과(box low-pass) 배경차감은 경계 클램프 아티팩트가 있다 — 다항식 피팅으로

**증상**: SEMU 무라 프록시에서 순수 선형 램프(그래디언트)가 semu=11.5로 나옴. 램프는 무라가
아니라 셰이딩이므로 ~0이어야 함(무라=국소 blob만 검출 대상).

**원인**: 배경을 대형 박스평균으로 추정 → 이미지 **가장자리에서 박스가 비대칭 클램프**되어
램프의 경계 열이 큰 잔차를 만듦. 이 경계 잔차가 넓은 면적(전체 높이 열)이라 SEMU를 부풀림.

**해결**: 배경을 **2차 다항식 표면 피팅(least-squares, {1,x,y,x²,xy,y²})**으로 교체.
선형 tilt/완만한 vignetting을 정확히 제거하면서 경계 아티팩트가 전혀 없다. 결과: 램프 semu
...
## 키 토글 요청의 대상 해석: "박스"는 이미 화면에 있는 그 박스다 (2026-07-31)

**수정받음**: "Ctrl+B로 block의 박스 바운더리를 토글" 요청을 전체 8x8 그리드 라인
오버레이로 구현했으나, 사용자 의도는 **이미 표시되던 worst-PSNR 블록 사각형**을
켜고 끄는 것이었다 (그리드 기능 자체는 마음에 들어 Ctrl+N으로 보존).

**왜 틀렸나**: "토글로 나타나고 사라지게"라는 표현에서 "현재 없는 것을 새로 그리는
기능"으로 점프했다. 하지만 토글 요청의 1순위 후보는 **이미 화면에 존재하는 요소**다.
기존 요소가 정확히 그 이름("block 박스")으로 존재하면 그것의 on/off가 기본 해석.

**적용법**: 표시 토글 요청을 받으면 먼저 "화면에 이미 있는 무엇을 끄고 싶은가?"를
검토하고, 새 오버레이 도입 해석이 더 그럴듯해 보이면 한 줄로 두 해석을 확인받거나
(사용자 부재 시) 기존 요소 토글을 기본으로 구현할 것.

## 2026-08-01 — 문서와 코드가 불일치할 때

- **증상**: 매뉴얼 집필용으로 소스를 전수 대조하니 치트시트·핫키 창·View 메뉴가
  코드와 어긋난 항목이 6건 나왔다 (`0` 키, `1~8` 배율 범위, `Ctrl+D`, JPEG EXIF,
  ini 경로, PNM 확장자).
- **교훈 1**: "문서를 코드에 맞춘다"가 항상 정답이 아니다. `0`=fit 표기가 3곳에
  독립적으로 있었다는 건 *원래 의도*가 그쪽이라는 증거였고, 사용자는 문서 수정이
  아니라 **키 재설계**(`Space`=fit, `0`=1:1)를 택했다. 불일치를 발견하면
  "어느 쪽이 의도인가"를 사용자에게 묻되, 선택지에 **코드 수정안**을 반드시 넣을 것.
- **교훈 2**: 중복 키(`0`과 `Space`가 둘 다 1:1)는 그 자체로 설계 냄새다.
  불일치를 고칠 때 중복도 같이 해소되는 안을 우선 제시할 것.
- **교훈 3 (typst)**: `#key("X")(...)` 처럼 함수 호출 바로 뒤에 `(` 가 오면
  두 번째 인자 목록으로 파싱되어 `expected comma` 로 깨진다. 한글 문장에서
  괄호 설명을 붙일 때 계속 재발했다 — 문서 수정 후에는 반드시 재컴파일할 것.
- **교훈 4 (배포)**: 세션이 끊겨 백그라운드 작업이 killed 되어도 **원격 작업은
  끝나 있을 수 있다**. 재시도 전에 원격 로그(`/tmp/wbuild.log`)와 산출물 mtime을
  먼저 확인하라 — 이번엔 빌드가 이미 성공해 있어 install 만 돌리면 됐다.
